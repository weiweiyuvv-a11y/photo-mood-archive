...
struct DayDetailView: View {
    ...
    var body: some View {
        ...
        .onChange(of: voiceRecorder.transcript) { _, transcript in
            ...
            draftNote = voiceBase + (voiceBase.isEmpty ? "" : "\n") + transcript
            archiveStore.setNote(draftNote, for: date)
        }
        .onDisappear { voiceRecorder.stop() }
        .onChange(of: activeEditor) { _, editor in
            if editor == nil { voiceRecorder.stop() }
        }
        .onChange(of: scenePhase) { _, phase in
            if phase == .background { voiceRecorder.stop() }
        }
    ...
    private var customNavigationRow: some View {
        HStack {
            ...
            .buttonStyle(.plain)
            .accessibilityIdentifier("add-memory")
        }
    }
        ...
        case .note:
            VStack(alignment: .leading, spacing: 14) {
                HStack {
                    ...
                    Button {
                        if !voiceRecorder.isRecording && !voiceRecorder.isStarting { voiceBase = draftNote }
    ...
    private var photoSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            if photos.isEmpty {
                ...
                .buttonStyle(.plain)
                .accessibilityIdentifier("open-photo")
                .padding(.top, 10)
...
private struct PhotoPreviewPagerView: View {
    ...
    var body: some View {
        ...
        .onChange(of: photoLibrary.allAssets.map(\.id)) { _, identifiers in
            let allowed = Set(identifiers)
            let previousIndex = visiblePhotos.firstIndex { $0.id == selectedPhotoID } ?? 0
            visiblePhotos.removeAll { !allowed.contains($0.id) }
            if visiblePhotos.isEmpty { onDismiss() }
            else if !visiblePhotos.contains(where: { $0.id == selectedPhotoID }) {
                selectedPhotoID = visiblePhotos[min(previousIndex, visiblePhotos.count - 1)].id
            }
        }
        .gesture(
...
private struct PhotoPreviewPage: View {
    ...
    var body: some View {
        ...
        .task(id: "\(photo.id)-\(photoLibrary.revision)") {
            ...
            do {
                image = try await photoLibrary.image(
                    for: photo,
                    size: CGSize(width: min(bounds.width * scale, 2400), height: min(bounds.height * scale, 2400)),
                    preview: true
                )
                hasLoaded = true
